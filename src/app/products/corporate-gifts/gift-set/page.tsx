...
import { ChevronRight, Star, Heart, Minus, Plus, ShoppingCart, Maximize2, FileDown, CheckCircle2 } from 'lucide-react';
...
const tiers = ["Standard", "Executive", "Premium"];
...
const relatedProducts = [
  { name: "Premium Gift Set", price: "from $45.00", minQty: "min 10", image: "https://placehold.co/310x220", badge: "Best Choice", category: "Corporate Gifts" },
  { name: "Executive Pen", price: "from $12.99", minQty: "min 50", image: "https://placehold.co/310x220", category: "Corporate Gifts" },
  { name: "Custom Mugs", price: "from $6.99", minQty: "min 12", image: "https://placehold.co/310x220", category: "Drinkware" },
  { name: "Notebook Set", price: "from $15.99", minQty: "min 20", image: "https://placehold.co/310x220", category: "Corporate Gifts" },
];

export default function ProductDetailsPage() {
  const [mainImage, setMainImage] = useState(productImages[0]);
  const [selectedTier, setSelectedTier] = useState('Executive');
      ...
      case "Description":
        return (
          <div className="flex flex-col gap-6 text-slate-600 text-xl font-normal font-['Inter'] leading-relaxed">
            <p>
              Impress your clients and team with our curated Premium Gift Set. Includes a custom-branded notebook, pen, and travel mug, all beautifully packaged in a high-quality box.
            </p>
            <h3 className="text-slate-900 font-bold mt-4">What's Included:</h3>
            <ul className="list-disc list-inside flex flex-col gap-2">
              <li>Custom Branded A5 Notebook (Hardcover)</li>
              <li>Matte Black Rollerball Pen with Laser Engraving</li>
              <li>12oz Insulated Stainless Steel Travel Mug</li>
              <li>Premium Presentation Box with Magnetic Closure</li>
            </ul>
          </div>
        );
      case "Ordering information":
        return (
          <div className="flex flex-col gap-4 text-slate-600 text-xl font-normal font-['Inter'] leading-8">
            <p>• Minimum order quantity for gift sets is 10 units.</p>
            <p>• Branding is included on all items within the set.</p>
            <p>• Lead time is typically 10-14 business days.</p>
      ...
      case "File Setup":
        return (
          <div className="flex flex-col gap-6 text-slate-600 text-xl font-['Inter'] leading-8">
            <h3 className="font-bold text-slate-900 mb-2">Design Specifications</h3>
            <p>Please provide your logo in vector format (AI or EPS) for the best engraving and printing results.</p>
          </div>
        );
      case "Template":
        return (
          <div className="flex flex-col gap-6">
            ...
            <div className="self-stretch px-4 py-4 rounded-[10px] border border-gray-300 flex justify-between items-center gap-4">
              <span className="text-slate-900 text-base font-bold font-['Inter']">Gift Set Branding Guide</span>
          ...
          <div className="flex flex-col gap-10">
            <div className="flex flex-col md:flex-row gap-10 items-center md:items-start">
              <div className="w-64 p-6 bg-green-50 rounded-2xl flex flex-col items-center justify-center gap-2">
                ...
                <span className="text-zinc-500 text-sm font-normal font-['Inter']">Based on 320 reviews</span>
              </div>
            </div>
          </div>
        );
    ...
    <main className="w-full bg-white flex flex-col items-center">
      ...
      <section className="w-full px-4 md:px-20 py-4 border-b border-gray-200">
        <div className="max-w-[1280px] mx-auto flex items-center gap-2 text-sm">
          ...
          <Link href="/products/corporate-gifts" className="text-emerald-500 hover:underline">Corporate Gifts</Link>
          <ChevronRight className="w-4 h-4 text-gray-400" />
          <span className="text-gray-600">Premium Gift Set</span>
        </div>
      </section>
      ...
      <section className="w-full max-w-[1280px] px-4 md:px-20 py-10 flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1 flex flex-col gap-6">
          <div className="flex flex-col gap-3">
            <h1 className="text-slate-900 text-4xl font-semibold font-['Outfit'] leading-tight">Premium Gift Set</h1>
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-500 text-xs font-normal font-['Inter']">(320 reviews)</span>
            </div>
            <p className="text-gray-600 text-sm font-normal font-['Inter'] leading-6">
              Impress your clients and team with our curated Premium Gift Set. High-quality items with your custom branding.
            </p>
          </div>

          {/* Tier Selection */}
          <div className="flex flex-col gap-3">
            <span className="text-slate-900 text-lg font-semibold font-['Inter']">Gift Tier</span>
            <div className="flex gap-3">
              {tiers.map((tier) => (
                <button
                  key={tier}
                  onClick={() => setSelectedTier(tier)}
                  className={`flex-1 min-w-[60px] h-12 rounded-[10px] font-bold font-['Inter'] border transition-all ${selectedTier === tier ? 'bg-green-700 text-white border-green-700' : 'bg-white text-gray-700 border-gray-300 hover:border-green-700'}`}
                >
                  {tier}
                </button>
              ))}
          ...
          <div className="flex flex-col gap-2.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-500 text-xs font-normal font-['Inter']">Min. 10</span>
            </div>
            <div className="w-40 h-12 bg-white rounded-lg border border-green-900/10 flex items-center overflow-hidden">
              <button onClick={() => setQuantity(Math.max(10, quantity - 1))} className="w-10 h-full flex items-center justify-center hover:bg-gray-50 transition-colors"><Minus className="w-4 h-4 text-neutral-700" /></button>
          ...
          <div className="flex flex-col gap-2">
            <div className="px-5 py-4 bg-green-50/50 rounded-xl flex items-center gap-4">
              <span className="text-green-700 text-3xl font-extrabold font-['Outfit']">$45.00</span>
              <div className="text-zinc-500 text-sm font-normal font-['Inter']">
                per unit · Total: <span className="text-green-700 font-bold">${(45.00 * quantity).toFixed(2)}</span>
              </div>
            </div>
          </div>
      ...
      <section className="w-full max-w-[1280px] px-4 md:px-20 py-10">
        <div className="bg-white rounded-2xl border border-gray-200 p-8 flex flex-col gap-8 shadow-sm">
          <div className="flex border-b border-gray-200 overflow-x-auto no-scrollbar">
            {tabs.map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-4 text-2xl font-medium font-['Outfit'] transition-all border-b-2 whitespace-nowrap ${activeTab === tab ? 'text-green-700 border-green-700' : 'text-zinc-500 border-transparent hover:text-green-700'}`}>{tab}</button>
            ))}
          </div>
          ...
          {relatedProducts.map((p, i) => (
            <div key={i} className="bg-white rounded-2xl shadow-sm border border-slate-900/5 flex flex-col overflow-hidden group hover:shadow-md transition-all">
              ...
              <div className="p-4 flex flex-col gap-3">
                ...
                <div className="flex items-center gap-1.5"><div className="flex gap-0.5">{[1,2,3,4,5].map(s => <Star key={s} className="w-3 h-3 text-orange-600 fill-orange-600" />)}</div><span className="text-slate-500 text-xs">4.9 (320)</span></div>
