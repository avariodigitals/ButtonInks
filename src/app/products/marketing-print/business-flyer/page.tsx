...
const paperTypes = ["Standard Matte", "Premium Gloss", "Recycled Matte"];
const flyerSizes = ["A4 (8.3\" x 11.7\")", "A5 (5.8\" x 8.3\")", "DL (3.9\" x 8.3\")"];
...
const relatedProducts = [
  { name: "Business Marketing Flyer", price: "from $15.99", minQty: "min 50", image: "https://placehold.co/310x220", badge: "High Quality", category: "Marketing Print" },
  ...
  { name: "Tri-fold Brochure", price: "from $25.99", minQty: "min 50", image: "https://placehold.co/310x220", category: "Marketing Print" },
  { name: "Promotional Poster", price: "from $9.99", minQty: "min 5", image: "https://placehold.co/310x220", category: "Marketing Print" },
];

export default function ProductDetailsPage() {
  const [mainImage, setMainImage] = useState(productImages[0]);
  const [selectedPaper, setSelectedPaper] = useState('Standard Matte');
  const [selectedSize, setSelectedSize] = useState('A4 (8.3" x 11.7")');
      ...
      case "Description":
        return (
          <div className="flex flex-col gap-6 text-slate-600 text-xl font-normal font-['Inter'] leading-relaxed">
            <p>Create a lasting impression with our high-quality marketing flyers. Perfect for promotional campaigns, event announcements, and menus. Available in a variety of paper weights and finishes.</p>
            <ul className="list-disc list-inside flex flex-col gap-2">
              <li>High-resolution digital offset printing</li>
              <li>Choice of matte, gloss, or uncoated stocks</li>
      ...
      case "Ordering information":
        return (
          <div className="flex flex-col gap-4 text-slate-600 text-xl font-normal font-['Inter'] leading-8">
            <p>• Minimum order quantity is 50 flyers.</p>
            <p>• Standard production time is 2-4 business days.</p>
            <p>• Digital proofs are provided for every custom order.</p>
          </div>
        );
      case "File Setup":
        return (
          <div className="flex flex-col gap-6 text-slate-600 text-xl font-['Inter'] leading-8">
            <h3 className="font-bold text-slate-900 mb-2">Bleed and Margin</h3>
            <p>Please include a 0.125" bleed on all sides and keep all text at least 0.25" from the edge.</p>
          </div>
        );
      case "Template":
        return (
          <div className="flex flex-col gap-6">
            ...
            <div className="self-stretch px-4 py-4 rounded-[10px] border border-gray-300 flex justify-between items-center gap-4">
              <span className="text-slate-900 text-base font-bold font-['Inter']">A4 Flyer Template</span>
          ...
          <div className="flex flex-col gap-10">
            <div className="flex flex-col md:flex-row gap-10 items-center md:items-start">
              <div className="w-64 p-6 bg-green-50 rounded-2xl flex flex-col items-center justify-center gap-2">
                ...
                <span className="text-zinc-500 text-sm font-normal font-['Inter']">Based on 580 reviews</span>
              </div>
            </div>
          </div>
        );
    ...
    <main className="w-full bg-white flex flex-col items-center">
      ...
      <section className="w-full px-4 md:px-20 py-4 border-b border-gray-200">
        <div className="max-w-[1280px] mx-auto flex items-center gap-2 text-sm">
          ...
          <Link href="/products/marketing-print" className="text-emerald-500 hover:underline">Marketing Print</Link>
          <ChevronRight className="w-4 h-4 text-gray-400" />
          <span className="text-gray-600">Business Marketing Flyer</span>
        </div>
      </section>
      ...
      <section className="w-full max-w-[1280px] px-4 md:px-20 py-10 flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1 flex flex-col gap-6">
          <div className="flex flex-col gap-3">
            <h1 className="text-slate-900 text-4xl font-semibold font-['Outfit'] leading-tight">Business Marketing Flyer</h1>
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-500 text-xs font-normal font-['Inter']">(580 reviews)</span>
            </div>
            <p className="text-gray-600 text-sm font-normal font-['Inter'] leading-6">
              Create a lasting impression with our high-quality marketing flyers. Perfect for promotional campaigns.
            </p>
          </div>

          {/* Size Selection */}
          <div className="flex flex-col gap-3">
            <span className="text-slate-900 text-lg font-semibold font-['Inter']">Flyer Size</span>
            <div className="flex flex-wrap gap-3">
              {flyerSizes.map((size) => (
                <button
                  key={size}
                  onClick={() => setSelectedSize(size)}
                  className={`flex-1 min-w-[100px] px-4 py-3 rounded-[10px] font-bold font-['Inter'] border transition-all ${selectedSize === size ? 'bg-green-700 text-white border-green-700' : 'bg-white text-gray-700 border-gray-300 hover:border-green-700'}`}
                >
                  {size}
                </button>
              ))}
          ...
          {/* Paper Type */}
          <div className="flex flex-col gap-3">
            <span className="text-slate-900 text-lg font-semibold font-['Inter']">Paper Type</span>
            <div className="flex flex-wrap gap-3">
              {paperTypes.map((paper) => (
                <button
                  key={paper}
                  onClick={() => setSelectedPaper(paper)}
                  className={`flex-1 min-w-[100px] px-4 py-3 rounded-[10px] font-bold font-['Inter'] border transition-all ${selectedPaper === paper ? 'bg-green-700 text-white border-green-700' : 'bg-white text-gray-700 border-gray-300 hover:border-green-700'}`}
                >
                  {paper}
                </button>
              ))}
          ...
          <div className="flex flex-col gap-2">
            <div className="px-5 py-4 bg-green-50/50 rounded-xl flex items-center gap-4">
              <span className="text-green-700 text-3xl font-extrabold font-['Outfit']">$15.99</span>
              <div className="text-zinc-500 text-sm font-normal font-['Inter']">
                per pack · Total: <span className="text-green-700 font-bold">${(15.99 * (quantity / 50)).toFixed(2)}</span>
              </div>
            </div>
          </div>
          ...
          {relatedProducts.map((p, i) => (
            <div key={i} className="bg-white rounded-2xl shadow-sm border border-slate-900/5 flex flex-col overflow-hidden group hover:shadow-md transition-all">
              ...
              <div className="p-4 flex flex-col gap-3">
                ...
                <div className="flex items-center gap-1.5"><div className="flex gap-0.5">{[1,2,3,4,5].map(s => <Star key={s} className="w-3 h-3 text-orange-600 fill-orange-600" />)}</div><span className="text-slate-500 text-xs">4.9 (580)</span></div>
