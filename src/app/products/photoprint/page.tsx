...
const subCategories = [
  { name: "Canvas Prints", image: "https://placehold.co/283x287" },
  { name: "Framed Photos", image: "https://placehold.co/283x287" },
  { name: "Photo Books", image: "https://placehold.co/283x287" },
  { name: "Acrylic Prints", image: "https://placehold.co/283x287" },
  { name: "Metal Prints", image: "https://placehold.co/283x287" },
];

const filterGroups = [
  {
    title: "Print Type",
    options: ["Canvas", "Acrylic", "Metal", "Paper", "Wood"]
  },
  {
    title: "Orientation",
    options: ["Landscape", "Portrait", "Square", "Panoramic"]
  },
  {
    title: "Finish",
    options: ["Glossy", "Matte", "Satin", "Lustre"]
  },
  {
    title: "Price Range",
      ...
      <section className="w-full p-6 bg-emerald-50 flex flex-col justify-start items-center gap-2.5">
        <div className="w-full max-w-[1280px] flex flex-col justify-center items-center gap-7 py-10">
          <nav className="px-3 py-2 rounded-md inline-flex justify-start items-center gap-1">
            ...
            <ChevronRight className="w-3.5 h-3.5 text-emerald-500" />
            <span className="text-center text-zinc-500 text-sm font-medium font-['Inter'] leading-5">Photoprints</span>
          </nav>
          <h1 className="self-stretch text-center text-green-500 text-6xl font-bold font-['Outfit'] leading-tight md:leading-[65.66px]">Photoprints</h1>
          <p className="w-full max-w-[480px] text-center text-zinc-500/90 text-base font-normal font-['Inter'] leading-6">Preserve your memories with professional photo printing. From classic paper prints to stunning acrylic wall art.</p>
        </div>
      </section>
          ...
          <div className="flex justify-start items-start gap-6 min-w-max">
            {subCategories.map((sub, index) => (
              <div key={index} className="flex-1 min-w-[220px] flex flex-col justify-start items-center gap-4 group cursor-pointer">
                <img className="self-stretch aspect-[283/287] object-cover rounded-2xl group-hover:shadow-md transition-shadow" src={sub.image} alt={sub.name} />
                <div className="self-stretch text-center text-neutral-950 text-base font-medium font-['Outfit'] group-hover:text-green-700 transition-colors">{sub.name}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          <aside className="w-full md:w-64 shrink-0 flex flex-col gap-4">
            <div className="p-5 bg-white rounded-2xl outline outline-[1.31px] outline-offset-[-1.31px] outline-green-900/5 flex flex-col gap-4">
              <div className="flex justify-between items-center pb-2 border-b border-green-900/5">
              ...
              {filterGroups.map((group, idx) => (
                <div key={idx} className="flex flex-col gap-3 py-4 border-t-[1.31px] border-green-900/5 first:border-t-0">
                  ...
                  <div className="flex flex-col gap-2">
                    {group.options.map((opt, i) => (
                      <label key={i} className="flex items-center gap-2 cursor-pointer group">
                        ...
                        <span className="text-neutral-700 text-xs font-normal font-['Inter'] group-hover:text-green-700 transition-colors">{opt}</span>
                      </label>
                    ))}
                  </div>
                </div>
                ...
                <div key={i} className="bg-white rounded-2xl shadow-[0px_2px_8px_0px_rgba(13,27,46,0.04)] outline outline-[1.31px] outline-offset-[-1.31px] outline-slate-900/5 flex flex-col overflow-hidden group hover:shadow-lg transition-all">
                  ...
                  <div className="p-4 bg-white border-t-[0.30px] border-zinc-500 flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      ...
                      <Heart className="w-5 h-5 text-zinc-500 hover:text-red-500 cursor-pointer transition-colors" />
                    </div>
