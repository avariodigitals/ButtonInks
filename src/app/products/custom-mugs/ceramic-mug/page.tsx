...
const relatedProducts = [
  { name: "Classic Ceramic Mug", price: "from $6.99", minQty: "min 12", image: "https://placehold.co/310x220", badge: "Bestseller", category: "Drinkware" },
  { name: "Travel Mug", price: "from $15.99", minQty: "min 12", image: "https://placehold.co/310x220", category: "Drinkware" },
  { name: "Premium Canvas Tote", price: "from $12.99", minQty: "min 25", image: "https://placehold.co/310x220", badge: "Eco-Friendly", category: "Bags" },
...
export default function ProductDetailsPage() {
  ...
  const [selectedCapacity, setSelectedCapacity] = useState('11oz');
      ...
      case "Description":
        return (
          <div className="flex flex-col gap-6 text-slate-600 text-xl font-normal font-['Inter'] leading-relaxed">
            <p>
              Brighten up your mornings with a custom-printed ceramic mug. Our high-definition printing process ensures your photos and logos look vibrant and sharp. Each mug is individually inspected to meet our quality standards.
            </p>
            <ul className="list-disc list-inside flex flex-col gap-2">
              <li>High-quality ceramic material</li>
              <li>Glossy finish for a premium look</li>
      ...
      case "Ordering information":
        return (
          <div className="flex flex-col gap-4 text-slate-600 text-xl font-normal font-['Inter'] leading-8">
            <p>• All products will be printed with the logo provided.</p>
      ...
      case "File Setup":
        return (
          <div className="flex flex-col gap-6 text-slate-600 text-xl font-['Inter'] leading-8">
            ...
            <p>Minimum 300 DPI at actual size. PNG with transparent background preferred.</p>
          </div>
        );
      case "Template":
        return (
          <div className="flex flex-col gap-6">
            ...
            <div className="self-stretch px-4 py-4 rounded-[10px] border border-gray-300 flex justify-between items-center gap-4">
              <span className="text-slate-900 text-base font-bold font-['Inter']">Standard Mug Print Template</span>
          ...
          <div className="flex flex-col gap-10">
            <div className="flex flex-col md:flex-row gap-10 items-center md:items-start">
              <div className="w-64 p-6 bg-green-50 rounded-2xl flex flex-col items-center justify-center gap-2">
                ...
                <span className="text-zinc-500 text-sm font-normal font-['Inter']">Based on 950 reviews</span>
              </div>
            </div>
          </div>
        );
    ...
    <main className="w-full bg-white flex flex-col items-center">
      ...
      <section className="w-full px-4 md:px-20 py-4 border-b border-gray-200">
        <div className="max-w-[1280px] mx-auto flex items-center gap-2 text-sm">
          ...
          <Link href="/products/custom-mugs" className="text-emerald-500 hover:underline">Custom Mugs</Link>
          <ChevronRight className="w-4 h-4 text-gray-400" />
          <span className="text-gray-600">Classic Ceramic Mug</span>
        </div>
      </section>
      ...
      <section className="w-full max-w-[1280px] px-4 md:px-20 py-10 flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1 flex flex-col gap-6">
          <div className="flex flex-col gap-3">
            <h1 className="text-slate-900 text-4xl font-semibold font-['Outfit'] leading-tight">Classic Ceramic Mug</h1>
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-500 text-xs font-normal font-['Inter']">(950 reviews)</span>
            </div>
            <p className="text-gray-600 text-sm font-normal font-['Inter'] leading-6">
              Our most popular mug. High-quality ceramic with a glossy finish. Microwave and dishwasher safe.
            </p>
          </div>
          ...
          {/* Capacity Selection */}
          <div className="flex flex-col gap-3">
            <span className="text-slate-900 text-lg font-semibold font-['Inter']">Choose Capacity</span>
            <div className="flex gap-3">
              {capacities.map((size) => (
                <button
                  key={size}
                  onClick={() => setSelectedCapacity(size)}
                  className={`flex-1 min-w-[60px] h-12 rounded-[10px] font-bold font-['Inter'] border transition-all ${selectedCapacity === size ? 'bg-green-700 text-white border-green-700' : 'bg-white text-gray-700 border-gray-300 hover:border-green-700'}`}
                >
                  {size}
                </button>
              ))}
          ...
          <div className="flex flex-col gap-2">
            <div className="px-5 py-4 bg-green-50/50 rounded-xl flex items-center gap-4">
              <span className="text-green-700 text-3xl font-extrabold font-['Outfit']">$6.99</span>
              <div className="text-zinc-500 text-sm font-normal font-['Inter']">
                per unit · Total: <span className="text-green-700 font-bold">${(6.99 * quantity).toFixed(2)}</span>
              </div>
            </div>
          </div>
          ...
          {relatedProducts.map((p, i) => (
            <div key={i} className="bg-white rounded-2xl shadow-sm border border-slate-900/5 flex flex-col overflow-hidden group hover:shadow-md transition-all">
              ...
              <div className="p-4 flex flex-col gap-3">
                ...
                <div className="flex items-center gap-1.5"><div className="flex gap-0.5">{[1,2,3,4,5].map(s => <Star key={s} className="w-3 h-3 text-orange-600 fill-orange-600" />)}</div><span className="text-slate-500 text-xs">4.9 (950)</span></div>
